...
const SelectSellerPage = () => {
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const remaining = getRemainingStockForSeller(selectedSeller.sellerId, selectedSeller.stock)
    if (val > remaining) {
      setError(
        `Only ${remaining} ${product.unit} available from ${selectedSeller.sellerName}`
      )
    } else {
      setError(null)
    }
...
const handleAddToCart = async () => {
    ...
    try {
      ...
      addNotification({                                         
        id: `cart-reminder-${Date.now()}`,                       
        title: "You're almost there!",                          
        body: `${product.name} is in your cart. Complete your order before your reservation expires.`, 
        read: false,                                            
