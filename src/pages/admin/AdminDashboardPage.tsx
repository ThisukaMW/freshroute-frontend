...
// ---------------------------------------------------------------------------
// Types (mirror TruckCapacityPage)
// ---------------------------------------------------------------------------
type Truck = {
  id: string
  operator: string
...
const AdminDashboardPage = () => {
  ...
  // -------------------------------------------------------------------------
  // Live fleet from localStorage (same source as TruckCapacityPage)
  // -------------------------------------------------------------------------
  ...
  useEffect(() => {
    ...
    const onStorage = (ev: StorageEvent) => {
      if (ev.key === 'fleet') setFleet(readFleet())
    }
  ...
  return (
    <div className="space-y-8 text-slate-100">
      ...
      {/* ------------------------------------------------------------------ */}
      {/* Fleet capacity — live from localStorage                             */}
      {/* ------------------------------------------------------------------ */}
      <section className="space-y-4 rounded-3xl border border-white/10 bg-gradient-to-br from-slate-950/70 via-supply-teal/10 to-slate-950/60 p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
              ...
              >
                ...
                <div className="grid gap-3 md:grid-cols-4">
                  {[
                    ...
                    {
                      label: 'Reefer',
                      value: truck.type.toLowerCase().includes('refrigerat') || truck.type.toLowerCase().includes('reefer')
                        ? truck.temperature
                        : '—',
                    },
                  ].map((item) => (
