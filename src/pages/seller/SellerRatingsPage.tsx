...
type View = 'products' | 'breakdown' | 'reviews'
...
const SellerRatingsPage = (): JSX.Element => {
            ...
            <button key={product.id} onClick={() => handleProductClick(product)}
              ...
              <div className="flex items-start gap-3">
                <div className="h-12 w-12 flex-shrink-0 rounded-xl bg-white/10 flex items-center justify-center text-xl">
                  {product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-cover rounded-xl" /> : '🛒'}
                </div>
  ...
  // ---- BREAKDOWN ----
  if (view === 'breakdown' && selectedProduct) return (
    <div className="max-w-4xl mx-auto space-y-6 px-4 py-6">
      ...
      <div className="flex items-center gap-3">
        <div className="h-12 w-12 flex-shrink-0 rounded-xl bg-white/10 flex items-center justify-center text-xl">
          {selectedProduct.imageUrl ? <img src={selectedProduct.imageUrl} alt={selectedProduct.name} className="h-full w-full object-cover rounded-xl" /> : '🛒'}
        </div>
        <div>
      ...
      {reviewsLoading ? (
        ...
        <>
                ...
                {[5, 4, 3, 2, 1].map(s => (
                  <div key={s} className="flex items-center gap-3 text-xs">
                    ...
                    <div className="flex-1 h-2 rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${((productStats.distribution[s] || 0) / productStats.total) * 100}%` }} />
                    </div>
          ...
          <button onClick={() => setView('reviews')}
            className="w-full rounded-2xl border border-emerald-500/30 bg-emerald-500/10 py-3 text-sm font-medium text-emerald-400 hover:bg-emerald-500/20 transition-colors">
            View all {productStats.total} individual reviews →
          </button>
        </>
      )}
    </div>
  ...
  // ---- INDIVIDUAL REVIEWS ----
  if (view === 'reviews' && selectedProduct) return (
    <div className="max-w-4xl mx-auto space-y-6 px-4 py-6">
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-supply-teal">Reviews</p>
        <h2 className="mt-0.5 text-2xl font-semibold text-slate-50">{selectedProduct.name}</h2>
        <p className="mt-1 text-sm text-slate-400">{reviews.length} review{reviews.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      <div className="space-y-3">
        {filteredReviews.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-white/5 p-8 text-center text-slate-400 text-sm">No reviews for this rating</div>
          ...
          <div key={r.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {r.isVerifiedPurchase && (
                      <span className="rounded-full bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 text-[10px] font-medium text-emerald-400">✓ Verified</span>
                    )}
            ...
            {r.comment && <p className="text-sm text-slate-300 border-t border-white/10 pt-2 leading-relaxed">"{r.comment}"</p>}
          </div>
        ))}
      </div>
    </div>
